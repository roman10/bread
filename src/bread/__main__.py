    ...
    try:
        ...
        if not skip_broker:
            try:
                ...
                typer.echo(
                    f"Warning: broker unavailable ({exc}); "
                    "skipping broker-side reset.",
                    err=True,
                )
...
def _load_strategy_universes(config: AppConfig) -> dict[str, list[str]]:
...
def _infer_strategy_from_symbol(
    universes: dict[str, list[str]], symbol: str,
) -> str:
...
def backfill_orders_cmd(
    ...
    try:
        ...
        if from_:
            try:
                after = datetime.strptime(from_, "%Y-%m-%d").replace(tzinfo=UTC)
            except ValueError as exc:
                typer.echo(f"Error: --from must be YYYY-MM-DD, got {from_!r}", err=True)
                raise SystemExit(1) from exc
        else:
            after = broker.get_account_created_at()
            typer.echo(f"--from omitted; using account creation date {after.isoformat()}")

        universes = _load_strategy_universes(config)
        ...
        with sf() as session:
            ...
            for idx, o in enumerate(alpaca_orders, start=1):
                ...
                strategy_name = _infer_strategy_from_symbol(universes, symbol)
